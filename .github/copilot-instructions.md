...
You are a senior developer working on a **Spring Boot** project using Java and Gradle.
...
- `docs/backend-architecture.md` - Detailed system architecture
- `docs/single-module-details-v1.md` - Module implementation details
- `docs/technical-details.md` - Technical specifications and stack
- `docs/tasks/user-stories.md` - User stories and requirements
...
- **Validation**: Leverage Spring's `@Validated` for input validation
- **Exception Handling**: Implement proper exception handling with `@ControllerAdvice`
- **Stereotypes**: Use appropriate Spring stereotypes (`@Service`, `@Repository`, `@RestController`)
- **REST API**: Follow RESTful conventions for API endpoints
- **Configuration**: Use `@ConfigurationProperties` for typed configuration
- **Transactions**: Apply `@Transactional` at service layer, not repository
