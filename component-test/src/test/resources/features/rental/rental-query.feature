...
Feature: Rental Query
  ...
  Scenario Outline: Get active rentals with status filter
    ...
    And the rental response only contains page of
      ...
      | 1  | CUS1       | 1           | 1        | ACTIVE | <startedAt> | <expectedReturnAt>  | 43         |
      | 2  | CUS2       | 2           | 1        | ACTIVE | <startedAt> | <expectedReturnAt2> | 0          |
  ...
  Scenario Outline: Get active rentals filtered by customerId
    ...
    And the rental response only contains page of
      ...
      | 3  | CUS1       | 3           | 1        | ACTIVE | <startedAt> | <expectedReturnAt> | 60         |
    And the response contains
      | path                 | value            |
      | $.totalItems         | 2                |
      | $.pageRequest.size   | 20               |
  ...
  Scenario Outline: Get active rentals with overdue calculation
    ...
    And the rental response only contains page of
      | id | customerId | equipmentId | tariffId | status | startedAt   | expectedReturnAt       | overdueMin       |
      | 1  | CUS1       | 1           | 1        | ACTIVE | <startedAt> | <overdueExpectedAt>    | <overdueMinutes> |
      | 2  | CUS2       | 2           | 1        | ACTIVE | <startedAt> | <notOverdueExpectedAt> | 0                |
    Examples:
  ...
  Scenario Outline: Get rentals filtered only by customerId
    ...
    Then the response status is 200
    And the rental response only contains page of
      | id | customerId | equipmentId | status    | startedAt   | expectedReturnAt   | overdueMin |
