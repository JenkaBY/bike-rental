...
2. Working tree contains no unrelated uncommitted changes you might sweep into the commit (note exclusions, e.g. local
   `application.yaml` tweaks — NEVER commit those).
3. Git branch: create `--branch` (default `feature/<req-id>-<fr>`), stacked on `--base` (default: current HEAD).
4. Postgres is up if the FR needs component tests (`docker ps`), and `JAVA_HOME` points to JDK 21 for every gradle run.
...
- Give it an EXPLICIT file list of the current sources it will prescribe edits for (facades, controllers, mappers,
  steps, transformers, changelogs, master changelog, `DbSteps`) — targeted Reads, not broad exploration; opus agents
...
- Remind it of the recurring review findings (these have all been rejected in code review before):
  step classes stateless / ScenarioContext-only; transformers instead of raw JSON or JsonPath for success bodies;
  Aliases for long constants; interface projections instead of JPQL constructor expressions to domain types;
  MapStruct mappers instead of hand-assembled DTOs (default methods for records); `@ConfigurationProperties` instead
  of hardcoded tunables; Accept-header content negotiation instead of format suffixes in URLs; new tables into
  `DbSteps.TABLE_TO_TRUNCATE`; NOT NULL columns populated in test seeds.
...
1. `./gradlew :service:test "-Dspring.profiles.active=test"` (includes `ModulithBoundariesTest`).
2. `./gradlew :component-test:test "-Dspring.profiles.active=test"` (DB must be up; Liquibase runs with
   `drop-first: true` in the component-test profile).
3. Fix failures YOURSELF (do not spawn agents for fixes); typical causes seen before: Spring Boot 4 package moves
   (`org.springframework.boot.resttestclient.TestRestTemplate`), MapStruct nested-record targets, stale-version
   visibility (needs `saveAndFlush`), cleanup-table list, unconditionally asserted response fields, fixture data that
   does not actually exercise the behavior (e.g. text short enough to fit one page).
