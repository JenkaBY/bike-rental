...
# Task 012: Create the SignAgreementService (one transaction, ordered steps)

> **Applied Skill:** `spring-boot-best-practices` — a single `@Transactional` method; `Clock` injected (never
> `Instant.now()`); cross-module calls go through facades only (`RentalSigningFacade`, `CustomerFacade`);
> `EventPublisher.publish(...)` inside the transaction (Modulith delivers after commit). `mapstruct-hexagonal` — DTO
> assembly delegated to `SigningAssemblyMapper`. Zero inline comments. Depends on Tasks 003, 004, 006, 007, 008, 009,
> 010, 011.

## 1. Objective

Implement the one-transaction signing orchestration in the EXACT ordered steps below. Ordering is load-bearing:
snapshot → template checks → duplicate check → fencing check → customer lookup → single-instant time → PDF render →
persist signature → synchronous `completeSigning` → publish event.

## 2. File to Modify / Create

* **File Path:** `service/src/main/java/com/github/jenkaby/bikerental/agreement/application/service/SignAgreementService.java`
...
> STEP ORDER IS LOAD-BEARING. Do NOT reorder: template checks before duplicate check before fencing check before
> customer lookup; the single `clock.instant()` is used for both `signedAt` and the PDF `startedAt`; the signature is
> saved BEFORE `completeSigning` so a `completeSigning` failure rolls the whole transaction back; the event is published
> LAST. Do NOT replace `clock.instant()` with `Instant.now()`. Do NOT catch `RentalNotAwaitingSignatureException`,
> `RentalSigningVersionMismatchException`, or `ObjectOptimisticLockingFailureException` — let them propagate to the
> advice.
