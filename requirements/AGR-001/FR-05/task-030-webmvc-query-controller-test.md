...
# Task 030: WebMvc test for RentalSignatureQueryController

> **Applied Skill:** `spring-mvc-controller-test` / `unit-tests` — `@ApiTest(controllers = ...)`, `@MockitoBean` for both
> use cases and the mapper, AssertJ-friendly `jsonPath` assertions, `@Nested` per behaviour. Covers content negotiation:
> empty JSON list, one-item JSON list, PDF attachment header, and 404 PDF when the use case throws
> `ResourceNotFoundException`. Depends on Tasks 010, 018, 019, 021.

## 1. Objective

Cover the GET controller: 200 empty JSON list; 200 JSON with one summary; 200 PDF with attachment `Content-Disposition`;
404 PDF when unsigned.

## 2. File to Modify / Create

* **File Path:** `service/src/test/java/com/github/jenkaby/bikerental/agreement/web/query/RentalSignatureQueryControllerTest.java`
...
```bash
./gradlew :service:test "-Dspring.profiles.active=test" --tests RentalSignatureQueryControllerTest
```
