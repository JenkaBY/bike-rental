...
# Task 020: Create the RentalSignatureCommandController

> **Applied Skill:** `spring-boot-best-practices` — thin `@RestController`, constructor injection, delegates to the use
> case; builds the command from the request + resolved ip/user-agent. `@Validated` at class level, `@Valid` on the body.
> Zero inline comments. Depends on Task 010, Task 018.

## 1. Objective

Create the POST controller at `/api/rentals/{rentalId}/signatures` returning `201 SignatureCreatedResponse`. It resolves
the client ip (first non-blank `X-Forwarded-For` token, else `request.getRemoteAddr()`) and the `User-Agent` header, and
passes both into the command.

## 2. File to Modify / Create

* **File Path:** `service/src/main/java/com/github/jenkaby/bikerental/agreement/web/command/RentalSignatureCommandController.java`
