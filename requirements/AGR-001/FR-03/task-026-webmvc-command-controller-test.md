...
# Task 026: WebMvc test for AgreementTemplateCommandController

> **Applied Skill:** `spring-mvc-controller-test` / `unit-tests` rule — use `@ApiTest(controllers = ...)`
> (never bare `@WebMvcTest`); `@MockitoBean` for every dependency; AssertJ; `@Nested` per method/status;
> request-validation NEGATIVE cases live HERE (blank/oversized title, blank content). Mirrors
> `customer/web/command/CustomerCommandControllerTest.java`.

## 1. Objective

Cover the command controller: 201 on valid POST, 200 on valid PATCH/activate, 204 on DELETE, and 400
for blank title, oversized title (> 255), and blank content on POST and PATCH. Depends on Task 016
(controller), Task 014 (DTOs), Task 010 (use cases), Task 015 (mapper).
