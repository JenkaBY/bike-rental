...
public interface FinanceFacade {
    ...
    HoldInfo holdFunds(@NonNull CustomerRef customerRef, @NonNull ActualRentalRef rentalRef,
                       @NonNull Money plannedCost, @NonNull String operatorId);

    boolean hasHold(RentalRef rentalRef);

    ReleaseHoldInfo releaseHold(@NonNull ActualRentalRef rentalRef, @NonNull String operatorId);
