---
name: Обновленная структура модуля Rental по архитектуре
overview: "Обновление структуры модуля Rental согласно backend-architecture.md с применением всех улучшений: Facade в корне модуля, разделение на Command/Query контроллеры, MapStruct, Repository Pattern, Value Objects, интерфейсы UseCase, ClockService, DomainEventPublisher, Long ID, ZonedDateTime, и точные эндпоинты из архитектуры."
todos: []
---
...
1. **Facade в корне модуля** (вместо `api/` пакета) - согласно нашему обсуждению
2. **Разделение контроллеров** на Command и Query - для уменьшения зависимостей
3. **Эндпоинты** точно по [docs/backend-architecture.md](docs/backend-architecture.md) (строки 531-539)
4. **Все предыдущие улучшения**: MapStruct, Repository Pattern, Value Objects, UseCase интерфейсы, ClockService,
   DomainEventPublisher, Long ID, ZonedDateTime
